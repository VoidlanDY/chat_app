...
class _FileManagerScreenState extends State<FileManagerScreen> {
  ...
  final Map<String, String> _downloadedFiles = {}; // fileName -> savePath
  ...
  void _applyFilter() {
    if (_selectedType == FileType.all) {
      ...
      _filteredFiles = _files.where((file) {
        ...
        return _getFileType(fileName) == _selectedType;
      }).toList();
    }
  }
  ...
  Future<void> _downloadFile(Message message) async {
    ...
    final chatService = context.read<ChatService>();
    final url = chatService.fixMediaUrl(message.mediaUrl);
  ...
  Widget _buildFileItem(Message file) {
    ...
    final progress = _downloadProgress[file.mediaUrl] ?? 0;
    final isDownloaded = _downloadedFiles.containsKey(fileName);
    final savedPath = _downloadedFiles[fileName];
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        Text(
                          _formatFileSize(file.mediaSize),
                          style: TextStyle(
                            color: Theme.of(context).colorScheme.onSurfaceVariant,
                            fontSize: 12,
                          ),
